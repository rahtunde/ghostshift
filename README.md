...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    GhostShift Platform (Docker)                     │
│                                                                     │
│  ┌─────────────────┐    ┌───────────────────┐    ┌───────────────┐ │
│  │  Nginx + React  │    │   Django + DRF    │    │   FastAPI     │ │
│  │   (Port :80)    │───▶│   (Port :8000)    │───▶│  AI Service   │ │
│  │                 │    │                   │    │  (Port :8001) │ │
│  │  Serves SPA     │    │  - REST API       │    │               │ │
│  │  Proxies /api/  │    │  - JWT Auth       │    │  - Burnout    │ │
│  │  to backend     │    │  - Django Q2      │    │    Scoring    │ │
│  │                 │    │  - Swagger Docs   │    │  - Risk Level │ │
│  └─────────────────┘    └─────────┬─────────┘    └───────────────┘ │
│                                   │                                 │
│                          ┌────────▼────────┐                        │
│                          │  PostgreSQL 16   │                        │
...
Docker is the easiest way to run GhostShift. One command starts the entire stack — PostgreSQL, Django backend, FastAPI AI service, and the React frontend — with no manual configuration.
...
1. Pull `postgres:16`, `python:3.12-slim`, `node:20-alpine`, and `nginx:1.27-alpine`
2. Build all three application images
3. Wait for PostgreSQL to be healthy before starting Django
4. Run `manage.py migrate` to set up all database tables
5. Seed the database with initial development data (Users, Departments, Shifts, Availability, Swaps, Burnout) and audit logs if the database is empty
6. Collect Django static files
7. Start all four services

> First build takes **2–4 minutes**. Every start after that is instant.
...
The following default accounts are seeded for testing:

| Role | Email | Password |
|---|---|---|
| **Admin** | `admin@ghostshift.test` | `Admin123!` |
| **HR** | `hr@ghostshift.test` | `HRpass123!` |
| **Manager** | `manager1@ghostshift.test` | `Manager123!` |
| **Employee** | `employee1@ghostshift.test` | `Employee123!` |

### Daily use — start and stop
...
### Management commands inside Docker

```powershell
# Seed core application data (Users, Departments, Shifts, Availability, Swaps, Burnout)
docker compose exec backend python manage.py seed

# Seed test data for the audit log
docker compose exec backend python manage.py seed_audit_logs

# Create a new admin account manually
...
# Start the Django Q2 background worker
docker compose exec backend python manage.py qcluster
```
